...
keys \refk{topsep} and \refk{partopsep} (perhaps as part of a style used in
every float box, defined with \refc{pseudodefinestyle}).
...
deck with \code*{no borderline}, and then re-do them all. So, for example, let's
say you want a thin bottom rule for your ruled style, more similar to the
original \emph{Concrete Mathematics} style \citep{Graham:1994}. You could do
that as follows:
\begin{texexp}
\begin{example}[pseudo/ruled,
    no borderline,
    bottomrule = \lightrulewidth,
    borderline north = {\heavyrulewidth}{0pt}{black},
    borderline south = {\lightrulewidth}{0pt}{black}]{\dots}{}
\dots
\end{example}
\end{texexp}
The title rule can still be changed by just using the \code{titlerule} key, as
in the previous example, if desired. Note that even though the box rules aren't
\emph{drawn}, they can still be used for spacing---which the borderlines don't
handle. In \code{pseudo/ruled}, \code{bottomrule} is set to \cs{heavyrulewidth},
so since we're replacing the bottom borderline to a thinner one, we need adjust
this as well.
...
you'll need borderline commands, again. For example:
\begin{texexp}
\begin{example}[pseudo/tworuled,
    no borderline,
    borderline horizontal = {\heavyrulewidth}{0pt}{lightgray}]{\dots}{}
\dots
\end{example}
\end{texexp}
...
\msg_new:nnn { pseudo } { useinstead } {
    The~#1 command~is~deprecated; \\ use~#2 instead.
}
