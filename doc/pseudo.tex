...
\begin{pseudo}[indent-mark, fullwidth]%*
    while $a \neq b$ & If equal, both are gcd \\+
        if $a > b$ & Reduce greatest with multiple of smallest \\+
            $a = a - b$ & $a$ is largest \\-
...
The actual pseudocode in \cref{alg:euclid} is typeset in the same way as on
\vpageref{p:intro-euclid}. The vertical lines are produced by the
\refk{indent-mark} option, and the comments are just a separate column of a
\code{tabularx}, which is used instead of the built-in \code{tabular}, as
explained in \cref{sec:tabularx}. (I.e., the \code{fullwidth} style that is
defined in \cref{sec:tabularx} is used in this example.) To get the surrounding
ruled box, a \pkg{tcolorbox} is used, with the style \code{pseudo/ruled}. This
has been set up with a predefined box environment, \code{algorithm}, which is
defined in \cref{sec:floats} (\pageref{p:pseudo-ruled-def}). The \refc{pr}
command and its relatives (as well as most other functionality) are discussed in
\cref{sec:overview}, with individual descriptions given in
\cref{sec:reference}.\footnote{The actual implementations, with explanations,
are found in \cref{sec:impl}.} The source of \cref{alg:euclid}, then, is the
following:
...
\cs{coloneqq} from \pkg{mathtools} (perhaps with
\verb|\let\gets\coloneqq|).\footnote{Tip: If you want to use a left-arrow
...
\pkg{float}.\footnote{Or you could do a quick
...
the \pkg{float} package, will give you many options, with varying
functionality.} You could also use the float environments supplied with packages
such as \pkg{algorithms}, \pkg{algorithmicx} and
\pkg{algorithm2e}.\footnote{The definition of \refc{==} doesn't properly carry
    over into floats. It's properly redefined inside \refe{pseudo}, so you
    probably won't notice, but if you wish to use the symbol outside the
    \refe{pseudo} environment, but in a float (e.g., inside \cs{caption}),
you'll need to either call \refc{pseudoeq} to re-establish \pkg{pseudo}'s
redefinition of \cs{=} or simply use \refc{eqs} instead of \refc{==}.}
...
\emph{boxed} and \emph{ruled} styles, as found in, e.g., \pkg{float}, as well as
...
to customize and extend the box style, \code{pseudo/boxruled} is probably the
...
is the header row, whereas in the \emph{Concrete Mathematics} style, it's the
caption.}
...
\definecolor{lighttan}{cmyk}{0,0.05,0.17,0} % from clrscode4e.sty
...
You can also new environments with \cs{newtcolorbox},\footnote{See the
...
etc.), \code{pseudo/ruled} is the ruled box style, and \code{alg} is is a prefix
...
% works.
\noindent
\Cref{pbx:boxed,pbx:boxruled,pbx:tworuled,pbx:filled} are typeset with the
remaining box styles.

\begin{internalpseudobox}[pseudo/boxed]{\code{pseudo/boxed}}{boxed}
\begin{pseudo}[kw]
    $i = 1$ \\
    while $i < n$\\+
        if \nf $i\== 1$ or $A[i-1] \leq A[i]$ \\+
...
\begin{internalpseudobox}[pseudo/boxruled]{\code{pseudo/boxruled}}{boxruled}
\begin{pseudo}[kw]
    $i = 1$ \\
    while $i < n$\\+
        if \nf $i\== 1$ or $A[i-1] \leq A[i]$ \\+
...
\begin{internalpseudobox}[pseudo/tworuled]{\code{pseudo/tworuled}}{tworuled}
\begin{pseudo}[kw]
    $i = 1$ \\
    while $i < n$\\+
        if \nf $i\== 1$ or $A[i-1] \leq A[i]$ \\+
...
\begin{internalpseudobox}[pseudo/filled]{\code{pseudo/filled}}{filled}
\begin{pseudo}[kw]%*
    $i = 1$ \\
    while $i < n$\\+
        if \nf $i\== 1$ or $A[i-1] \leq A[i]$ \\+
...
\code{tcolorbox} is \code{pseudo/boxruled}, which may be a good starting-point
...
\code{pseudo/filled}), starting with that style may be easier.
...
``Require/Ensure'' and ``Data/Result.''}
...
\begin{docCommand}{==}{}
    \pseudoeq % Having some trouble bc/o floats, or something?
    ...
    the \refc{==} shortcut) by using \refc{pseudoeq}. In some cases, you may
    want to just use \refc{eqs} instead.
\end{docCommand}
...
\begin{docCommand}{dts}{} % The name in Concrete Mathematics
    ...
    in \pkg{gkpmac}. Also accessible via the \refc{..} shortcut.
\end{docCommand}
...
\begin{docCommand}{pseudoslash}{}
    Command similar to the \cs{arrayslash} of the \pkg{array} package. Switches
    the definition of \cs{\bslash} to the one used by \pkg{pseudo}. Useful if
    you've used some code that modifies \cs{\bslash} for its own purposes (such
    as \cs{raggedleft} or the like).
\end{docCommand}
...
by adding \code{>\braces{\refc{pseudoeq}}} before each column.\footnote{You can
also, of course, just use \refc{eqs} instead.}
...
over the indent levels, inserting one indent marker for each leve.
\begin{source}
\cs_new:Nn \@@_indent:N {
...
command from Concrete Mathematics (see \pkg{gkpmac.tex}).
...
\end{source}
The default \refk{indent-mark} is a vertical with width set by
\refk{indent-mark-width}. The height and depth are normally
\code{.7\cs{baselineskip}} and \code{.3\cs{baselineskip}}, though these are
scaled by \code{line-height}. This is simply handled by measuring the array
strut (as given by \code{\@arstrutbox}),\footnote{Ideally, we should be using
\cs{rule:nnn}, here, but that is a fairly recent command, and not
necessarily available.} which will only work for tabular environments (as set by
\refk{begin-tabular} and \refk{end-tabular}) that use \code{@arstrutbox}. If
there is trouble with this, a simple fix would be to set \refk{indent-mark} to
\code*{\cs{vrule} height .7\cs{baselineskip} depth .3\cs{baselineskip} width
.6pt},\footnote{Or
\code*{\cs{rule}[-.3\cs{baselineskip}]\braces{.6pt}\braces{\cs{baselineskip}}}.}
changing or scaling the values as appropriate.
\begin{source}
    ...
    indent-mark-width  .initial:n   = .6pt, % TikZ semithick
...
\end{source}
\noindent
The \refc{pseudoslash} command simply redefines the row separator, and is used
at the start of the \refe{pseudo} environment. It may be useful for the user if
some other construct redefines \refc{\bslash} as well. (This is similar to the
\cs{arraycr} command of the \pkg{array} package.)
\begin{source}
...
\pgfqkeys {tcb/pseudo} {
\end{source}
The first style (\code{pseudo/boxruled}) is the basis for the others.
\begin{source}
boxruled/.style = {
\end{source}
...
\end{source}
Spacing before and after the box, when not used with \code{float}:\footnote{This
setting is supposed to be the \pkg{tcolorbox} default; however, though in some
versions, this default does not work (cf.\@
\url{https://github.com/T-F-S/tcolorbox/issues/182}).}
\begin{source}
...
``body'' of the box, we add some extra space.
\begin{source}
    boxsep              = 0pt,
    toptitle            =      \belowrulesep,
    bottomtitle         =      \aboverulesep,
    top                 = 1.75 \belowrulesep,
    bottom              = 1.75 \aboverulesep,
    left                = 1.50 \belowrulesep,
    right               = 1.50 \belowrulesep,
\end{source}
...
\end{source}
Finally, some basic styling. The values \code{.4pt} and \code{.2pt} correspond
to the \pkg[https://ctan.org/pkg/pgf]{tikz} line thicknesses \code{thin} and
\code{very thin}.
\begin{source}
    sharp~corners,
    colback             = white,
...
(\cs{empty} and \cs{tile}), which remove the default frame drawing, as some of
the frame are removed. (It would be possible to simply set the relevant widths
to zero, but this tends to leave perceptible hairlines in most PDF
...
being the first paragraph of the contents, so we zero out the spacing after the
title, and make the top spacing equal to the the normal \cs{parskip} (which we
set to \code{.3\cs{baselineskip}} in \code{before upper}).
\begin{source}
boxed/.style = {
    pseudo/boxruled,
    empty,
    titlerule           = 0pt,
    borderline          = {.4pt}{0pt}{black},
    bottomtitle         = 0pt,
    top                 = .3\baselineskip,
},
\end{source}
...
\code{boxruled} is a bit redundant, here.}
\begin{source}
filled/.style = {
    pseudo/boxruled,
    tile,
