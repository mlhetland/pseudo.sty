...
combined with the row separator \refc{\bslash}). Here, once again, is Euclid's
algorithm for finding the gcd of $a$ and $b$:
...
\end{texexp}
%
The default indent mark scales with the line height, to avoid gaps in the
vertical lines, and its width has no impact on the indentation. The width of the
(default) mark can be set with \refk{indent-mark-width}:
\begin{texexp}
\begin{pseudo}[
...
%
    ...
    See also the \refk{extra-space} key, which may be used to insert extra
    vertical space as part of the line break introduced by \refc{\bslash}.
\end{docCommand}
...
\begin{source}
    indent-mark     .default:n      =
        ...
        \skip_horizontal:n {-\l_@@_indent_mark_wd_tl},
