...
\raggedbottom % All the boxes lead to a lot of stretching ...
...
\DeclareTCBListing{texexp}{ !O{} }{ texexp, #1 }

% Broken by this:
% https://tug.org/pipermail/latex3-commits/2023-September/031580.html
% For now (2025-01-29), at least, switching to `tcboutputlisting' and friends.
% \ExplSyntaxOn
% \NewDocumentCommand \ExecutedListing { +v } {
%     \group_begin:
%     \obeylines
%     \exp_args:Nx \scantokens {
%         \string\begin{texexp}[listing]
%             #1
%         \string\end{texexp}
%     }
%     \group_end:
...
The pseudocode in \cref{alg:euclid} is typeset in the same way as
...
mark at every indentation step:
\begin{texexp}
...
The basic formatting commands are not used in the internals of the package, so
they may be freely redefined for different styling, such as
\verb|\let\id\textsf|. They generally do some extra work, though, such as
wrapping the styled text in \cs{textnormal} to avoid having the styles blend,
adding quotes (\cs{st}) and handling parenthesized arguments (\cs{pr}). To let
you hook into their appearance without messing with their definitions, each
command has a corresponding font command (\cs{kwfont}, \cs{cnfont}, \cs{idfont},
etc.), which you may redefine. These fonts may even be set using correspondingly
named options, either with \refc{pseudoset} or via optional keyword arguments to
the \refe{pseudo} environment:\footnote{Because of \LaTeX\ expansion behavior,
they can \emph{not} be set globally when importing \pkg{pseudo}.}
\begin{texexp}
...
\noindent
This star also works after \code{\cs{begin}\braces{pseudo}}. As mentioned, in
...
Add the following to your preamble to define an \code{algorithm} float:
\begin{texexp}[listing]
...
``\href{https://ctan.org/search/?phrase=theorem}{theorem}''. The \pkg{tcolorbox}
approach of \pkg{pseudo} can also be adapted to this style.}

Using the \refe{pseudo} environment is sufficient for typesetting pseudocode as
part of the body text. If you wish to place your pseudocode in a float, you can
easily use a package such as
\pkg[https://ctan.org/tex-archive/macros/latex/contrib/float]{float}. or the
float environments from packages like \pkg{algorithms}, \pkg{algorithmicx} and
\pkg{algorithm2e}.
\footnote{Or you could do a quick \href{https://ctan.org}{\textsc{ctan}} search
for ``\href{https://ctan.org/search/?phrase=float}{float}'', or take a look at
the \href{https://ctan.org/recommendations/float}{recommendations} related to
the \pkg[https://ctan.org/tex-archive/macros/latex/contrib/float]{float}
package, which will give you many options, with varying functionality.}

\begin{note}
The definition of \refc{==} doesn't properly carry over into floats. It's
redefined inside \refe{pseudo}, but if you wish to use it outside the
\refe{pseudo} environment, but in a float (e.g., inside \cs{caption}), you'll
need to call \refc{RestorePseudoEq} to re-establish \pkg{pseudo}'s redefinition
of \cs{=} or simply use \refc{eqs}.
\end{note}
...
the \emph{boxruled} and \emph{tworuled} styles found in \pkg{algorithm2e}
In addition, there's a \emph{filled} style, with a colored background. (Examples
of the styles are found
\vpageref{alg:gnome,pbx:booktabs,pbx:boxed,pbx:boxruled,pbx:tworuled,pbx:filled}.)
If you wish to customize and extend the box style, \refk{pseudo/boxruled} is
probably the best starting point, as the other styles disable the default frame
drawing.
...
environments, possiby with additional \pkg{tcolorbox} options for
customization:\footnote{The actual \pkg{clrscode4e} background is
\code{\cs{color}[cmyk]\braces{0,0.05,0.17,0}}.}
...
\begin{tcolorbox}[pseudo/filled, colback=sbase3]
    \begin{pseudo}*
...
% \begin{note}
% Here I've used \code{base3} from \pkg{xcolor-solarized} as the background. The
% actual \pkg{clrscode4e} color is defined as follows:
% \begin{texexp}[listing]
...
\begin{docKey}{pos}{\,=\,\meta{position}}{\initialkeyvalue{\code{t}}}
...
\pkg[https://ctan.org/tex-archive/macros/latex/contrib/pseudocode]{pseudocode}
uses large braces. In \pkg{pseudo}, you can use an \refk{indent-mark}, for which
the default is a semithick, gray vertical line (see \cpageref{p:indent-mark}).
However, with \pkg[https://ctan.org/pkg/pgf]{tikz}, you could draw all kinds of
indentation decorations, for example using \pkg{tikzmark} to anchor the drawings
at the individual lines.

\begingroup

For example, you might add a \cs{tikzmarknode} at the start of each code line.
This might contain a small zero-width box covering the vertical extent of normal
text (such as \code{\cs{vphantom}\braces{fg}}). The nodes can be given names
like \code{line-1}, etc., using \code{\cs{arabic}\braces{\refk{*}}} for the
numbering:

\begin{tcboutputlisting}
% In document preamble:
% \usepackage{tikz}
% \usetikzlibrary{tikzmarks}

\def\shortstrut{\vphantom{fg}}

\pseudoset{
    setup-append = {\tikzmarknode{L\arabic*}{\shortstrut}}
}
\end{tcboutputlisting}
\tcbinputlisting{texexp, listing}
\tcbuselistingtext

\begingroup

\noindent
For each \refe{pseudo} environment, we now get
\pkg[https://ctan.org/pkg/pgf]{tikz} anchors like \code{L1.north}, etc., which
can be used for drawing. We can define a custom command for drawing the indent
marks, relying on the \pkg[https://ctan.org/pkg/pgf]{tikz} \cs{foreach} syntax,
supplying the line number intervals as \code{2/5}, etc.
\begin{note}
    The \code{overlay} and \code*{remember picture} options to the \cs{tikz}
    command below are needed. To shift the line to the left, \code{xshift} is
    convenient, but because we are using named points, it must be combined with
    \code*{transform canvas}.
\end{note}

\begin{tcboutputlisting}
\tikzset{indent mark/.style = {semithick, lightgray}}

\NewDocumentCommand \DrawIndentMarks { m m } {
    \tikz [
        overlay, remember picture,
        transform canvas={xshift=-1.25em}
    ] {
        \foreach \i/\j in {#1} \draw[indent mark]
            (L\i.north) -- (L\i.north |- L\j.south);
        \foreach \i/\j in {#2} \draw[indent mark]
            (L\i.north) -- (L\i.north |- L\j.west) -- +(.5em,0);
    }
}
\end{tcboutputlisting}
\tcbinputlisting{texexp, listing}
...
\noindent
This command takes two arguments, where the first is a list of intervals that
end with some ``continuing'' keyword, such as \kw{else}, while the second is a
list of intervals that terminate with the end of a statement. These latter ones
loosely emulate the terminating ``hook'' used in, e.g.,
\pkg{algorithm2e}.

\begin{note}
I've centered the hook on the final line to illustrate the use of different
anchors. If you'd rather have it lower, you could replace \code{L\cs{j}.west} by
\code{L\cs{j}.south}.
\end{note}

\noindent
To not mess up the
spacing around the \refe{pseudo} environment, we can put the drawing command
inside it, at the end.

\begin{texexp}
\begin{pseudo}[kw, indent-length=1.5em]
$p = 1$                                                       \\
while $n > 1$                                                 \\+
    if $n \bmod 2 \== 0$                                      \\+
        $x = x \cdot x$                                       \\
...
\noindent
Braces somewhat similar to those used by the \pkg{pseudocode} package could be
drawn by altering our definitions slightly:
...
\tikzset{
    indent mark/.style = {
        thick, decorate,
        decoration = {calligraphic brace, mirror, raise=.3em},
    },
    mark label/.style = {
        midway, font=\shortstrut, left=.5em, anchor=east
    }
}
\end{tcboutputlisting}
\tcbinputlisting{texexp, listing}
...
I've also added a format for labels (like ``then'' or ``else'', etc.), which can
be added to the list being iterated over. These are drawn to the left of the
braces:

\begin{tcboutputlisting}
\NewDocumentCommand \DrawIndentMarks { m } {
    \tikz [overlay, remember picture] {
        \foreach \i/\j/\label in {#1} \draw[indent mark]
            (L\i.north) -- (L\i.north |- L\j.south)
            node[mark label] {\label};
    }
}

% For left-alignment with "then":
\NewDocumentCommand \Else { } {\rlap{else}\phantom{then}}
\end{tcboutputlisting}
\tcbinputlisting{texexp, listing}
\tcbuselistingtext

\noindent
With this style, we might not want line numbers. However, if we use the
\refe{pseudo*} environment (or the \refk{starred} style), we'll lose the line
number counter required to name our \pkg[https://ctan.org/pkg/pgf]{tikz} nodes!
Instead, we can simply \emph{hide} the line numbers, using \refk{label} and
\refk{hsep}:

\begin{texexp}
\begin{pseudo}[
    line-height=1.1, indent-length=3.5em, label={}, hsep=0pt]
$p = 1$                                                       \\
while $n > 1$                                                 \\+
    if $n \bmod 2 \== 0$                                      \\+
        $x = x \cdot x$                                       \\
        ...
        $n = n - 1$                                           \\--
$p = p \cdot x$
\DrawIndentMarks{3/7/do, 4/5/then, 6/7/\Else}
\end{pseudo}
\end{texexp}

\endgroup

In these examples, the actual drawing of the brace or line (or whatever) isn't
automated, of course. This could be done by some hook triggered by the \code{-}
flags in \refc{\bslash}. If it turns out there's a demand for something like
that, I might add something like that in a future version.
...
\end{source}
We'll also create a couple of globals, to help us get some key-setting results
out of the group surrounding the code handling unknown keys:
\begin{source}
...
\keys_define:nn { pseudo } {
    unknown .code:n = {
        ...
        % We'll modify this globally, remporarily, and put it back
...
\begin{source}
        ...
        % the group:
        \tl_set_eq:NN \l_@@_extra_space_tl \g_@@_extra_space_temp_tl
